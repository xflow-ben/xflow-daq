...
crosstalk.loads_names = {'Tower_Top_Fx','Tower_Top_Fy','Tower_Top_Mx','Tower_Top_My','Rotor_Torque'}; % Names of physical loads of intrest which are applied during calibrations
...
calib(calibNum).applied_load_scaling = cosd(horz_angle).*[sind(theta); cosd(theta); sind(theta)*consts.heights.tower_top; cosd(theta)*consts.heights.tower_top; zeros(size(theta))];
...
horz_angle = [(-9.3+-10.6)/2, (-10.1+-11.5)/2,...
...
horz_angle = [-11.9, -12.3, -11.8, -11.9];
...
calib(calibNum).applied_load_scaling = [0; 0; sind(theta)*consts.rotor.radius; cosd(theta)*consts.rotor.radius; zeros(size(theta))];
...
% 
% % Calculated inputs
% calib(calibNum).applied_load_scaling = [0; 0; 0; 0; -cosd(consts.blade.pitch_30)*consts.rotor.radius*consts.units.lbf_to_N];
% 
% %% Rotor torque, +X
% calibNum = calibNum + 1;
% calib(calibNum).folder = 'installed_rotor\+X';
