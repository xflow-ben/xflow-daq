...
calib(calibNum).applied_load_scaling = cosd(angle).*[sind(theta), cosd(theta), sind(theta)*consts.heights.tower_top, cosd(theta)*consts.heights.tower_top, zeros(size(theta))];
...
% Calculated inputs
calib(calibNum).applied_load_scaling = [0, 0, 0, 0, -consts.rotor.radius*consts.units.lbf_to_N];
