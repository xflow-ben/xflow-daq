...
d.task(i).addChannel('AIVoltage','cDAQ9188-18F21FFMod6','ai17','Battery Voltage','RSE',[0 10]); % might need to move to acc channel if module can't do 2 tasks
...
sampleClockTimebaseRate = task(i).rate*256*31;
d.routeSignal('/cDAQ9184-1A4BA5D/20MHzTimebase','/cDAQ9184-1A4BA5D/PFI0')
d.task(i).setSampClkTimebaseSrc('/cDAQ9184-1A4BA5D/PFI0');
d.task(i).setSampClkTimebaseRate(sampleClockTimebaseRate);
...
d.createTask('CI',task(i).name,task(i).rate,task(i).startOrder);
d.task(5).addChannel('CIAngEncoder', 'cDAQ9184-1A4BA5DMod1', 'ctr0', 'Encoder','PFI0','PFI1','PFI2','X4',encPPR,'AHighBHigh');
...
% route gw_strain start trigger to PFI0, which is physically 
