...
d.addAIVoltageChan('cDAQ9188-18F21FFMod6/ai0','Diff',[-10 10]);


bufferSize = 255;

% reserve
err = calllib('myni','DAQmxTaskControl',d.tasks(1).taskHandle,d.DAQmx.Val_Task_Reserve);

[err,~,stringOut] = calllib('myni','DAQmxGetSampClkTerm',d.tasks(1).taskHandle,blanks(bufferSize),uint32(bufferSize));
[err,~,stringOut] = calllib('myni','DAQmxGetStartTrigTerm',d.tasks(1).taskHandle,blanks(bufferSize),uint32(bufferSize));


[int32, voidPtr, cstring] DAQmxGetStartTrigTerm(voidPtr, cstring, uint32)
DAQmxGetStartTrigTerm
DAQmxSetStartTrigType
DAQmxSetRefTrigType

DAQmxSetStartTrigType(TaskHandle taskHandle, int32 data);



DAQmxGetSampClkTerm
analogTask.createChannel('AnalogInputVoltage', 'cDAQ1Mod2/ai0', 'AnalogChan', d.DAQmx_Val_RSE, -10.0, 10.0, d.DAQmx_Val_Volts);
